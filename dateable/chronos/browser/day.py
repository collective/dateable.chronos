...
class DayView(BaseCalendarView):
    ...
    def calcInfo(self):
        ...
        calendar_tool = getToolByName(self, 'portal_calendar')
        hours_to_display = calendar_tool.getHoursToView()
        self.from_hour = hours_to_display[0]
        self.to_hour = hours_to_display[1]
    ...
    def getEventDisplays(self):
        ...
        start = datetime.combine(self.today, time(0,0))
        end = datetime.combine(self.today + timedelta(1), time(0,0))
        
        occurrences = provider.getOccurrences(start, end)
