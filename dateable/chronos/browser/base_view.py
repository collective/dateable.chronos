...
class BaseCalendarView(object):
    ...
    def _set_default_day(self, defaultday):
        ...
        self.calendar_tool.setViewDay(defaultday)
    ...
    def _get_default_day(self):
        ...
        if not hasattr(self, 'request'):
            self._default_day = self.calendar_tool.getViewDay()
            return self._default_day
        ...
        if month is None:
            return self.calendar_tool.getViewDay()
        
        year = year or self.calendar_tool.getViewDay().year
        year = int(year)
        month = int(month)
    ...
    def view_tabs(self):
        ...
        from Products.CMFonFive.fiveactionstool import getMenu
        menu = getMenu('chronos_views', self.context, self.request)
        return menu
