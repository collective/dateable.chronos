...
class BaseCalendarView(object):
    ...
    def localizedHour(self, hour):
        siteProperties = getToolByName(self.context, 'portal_properties').site_properties
        try:
            localTimeFormat = siteProperties.localTimeOnlyFormat
        except AttributeError:  # < Plone 3.2 compatibility
            localTimeFormat = utils.localTimeFormatFromLocalDateTimeFormat(siteProperties.localLongTimeFormat)
        return strftime(localTimeFormat, (0, 1, 1, hour, 0, 0, 0, 1, -1))
